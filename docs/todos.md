...
1. Screen 1 — Library: track rows (status dot, measured loudness/peak,
...
2. Screen 2 — Chain view: interactive module rack exposing `master.py`'s
   ...
   set.
3. Meter sidebar showing the real measured LUFS/true-peak/ladder parsed
   from `master.py`'s post-run output (see scoping note above on what
   "live" means here).
4. Screen 3 — Compare: before/after cards using `libraryAnalyze`'s real
   ...
   (`#screen-compare`).
5. Screen 4 — Export: batch export queue reusing the existing
...
6. Save/reuse named chain presets ("Save presets used" in the mockup) —
   extend `src/main/library.js`'s store to remember a named parameter set a
   user can reapply to other tracks.

## Distribution

7. Real code signing + notarization (requires an Apple Developer ID
