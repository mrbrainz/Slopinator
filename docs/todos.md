...
1. In-app audio playback (local file paths) — needed for Chain view's
   waveform and Compare's A/B listen buttons. Nothing plays audio today.
2. Screen 1 — Library: track rows (status dot, measured loudness/peak,
...
3. Screen 2 — Chain view: interactive module rack exposing `master.py`'s
   ...
   `window.slopinator.getPeaks()`. On a successful master run,
   `libraryUpdate` the track to `status: 'mastered'` with
   `masteredPreset`/`masteredAt` set.
4. Meter sidebar showing the real measured LUFS/true-peak/ladder parsed
   from `master.py`'s post-run output (see scoping note above on what
   "live" means here).
5. Screen 3 — Compare: before/after cards using `libraryAnalyze`'s real
   original-file stats and #1's real playback, replacing the mockup's
   static numbers. Currently a placeholder (`#screen-compare`).
6. Screen 4 — Export: batch export queue reusing the existing
...
7. Save/reuse named chain presets ("Save presets used" in the mockup) —
   extend `src/main/library.js`'s store to remember a named parameter set a
   user can reapply to other tracks.

## Distribution

8. Real code signing + notarization (requires an Apple Developer ID
