...
// see package.json), so a running/packaged app's footer can be matched
// back to an exact commit instead of guessing whether it's stale. Read
// here (main process, never sandboxed) and handed to the renderer over
// IPC — preload.js runs under Electron's default sandboxed preload
// (Electron 20+), which has no generic require('fs')/require('path').
const buildInfo = (() => {
  try {
...
function buildMasterArgs(params = {}) {
  ...
  if (params.saturationCrossover != null) args.push('--saturation-crossover', String(params.saturationCrossover));
